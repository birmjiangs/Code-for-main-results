		...
		for i in range(len(base_chrN)):
			...
			for j in range(data_start,len(data_chrN)):
				...
				if D[2] < B[1] and cache_largest_data_end_point < B[1]:
					data_start = j  #bugged, may exist a data so long that the D[2] of the following data < B[1] but the D[2] of this data is not < B[1], and the bug will lead to a skip of this long data
